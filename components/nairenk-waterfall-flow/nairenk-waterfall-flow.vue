<template>
	<view class="flow-box padding bg-white" :style="'height: ' + loadingTop + 'px'">
		...
		<view class="flow-item1" v-if="modelType == 1" :class="left[index] == 1 ? 'left' : ''" :style="'top:' + top[index] + 'px;'"
		...
		<view class="flow-item2" v-if="modelType == 2" :class="left[index] == 1 ? 'left' : ''" :style="'top:' + top[index] + 'px;'"
		 v-for="(item, index) in newList" :key="index" :data-index="index" @click="choose">
			<view class="pic">
				<image class="image" mode="widthFix" :src="item.image" style="width: 100%; display: block;"></image>
			</view>
