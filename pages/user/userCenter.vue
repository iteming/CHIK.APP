<template>
	<view>
		<view class="header" v-bind:class="{'status':isH5Plus}">
			...
			<view class="setting">
				<image src="../../static/userCenter/setting.png"></image>
			</view>
		</view>
		<view class="orders">
			<view class="box">
				<view class="label" v-for="(row,index) in orderTypeLise" :key="row.name" hover-class="hover" @tap="toOrderType(index)">
					<view class="icon">
						...
						<image :src="'../../static/userCenter/'+row.icon"></image>
					</view>
					{{row.name}}
				</view>
			</view>
		...
		<view class="list" v-for="(list,list_i) in severList" :key="list_i">
			<view class="li" v-for="(li,li_i) in list" @tap="toPage(list_i,li_i)" v-bind:class="{'noborder':li_i==list.length-1}"
			 hover-class="hover" :key="li.name">
				<view class="icon">
					<image :src="'../../static/userCenter/sever/'+li.icon"></image>
				</view>
				<view class="text">{{li.name}}</view>
				<image class="to" src="../../static/userCenter/to.png"></image>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			init() {
				//用户信息
				this.userinfo = {
					face: '../../static/userCenter/face.jpeg',
					username: "VIP会员10240",
					integral: "1435"
				}
			},
